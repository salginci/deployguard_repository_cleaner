...
from deployguard.core.models import Severity, Finding, SecretType
    ...
    try:
        ...
        if include_history:
            ...
            if git_dir.exists():
                ...
                # Convert SecretMatch objects to Finding objects for unified reporting
                for secret in history_secrets:
                    # Create a Finding for each unique secret
                    finding = Finding(
                        type=SecretType(secret.secret_type) if secret.secret_type in [e.value for e in SecretType] else SecretType.GENERIC_SECRET,
                        severity=Severity(secret.severity) if secret.severity in [e.value for e in Severity] else Severity.HIGH,
                        file_path=secret.files[0] if secret.files else "git-history",
                        line_number=0,
                        description=f"Secret found in {len(secret.commits)} commits across {len(secret.files)} files",
                        exposed_value=secret.value,
                        suggested_variable=secret.suggested_env_var,
                        metadata={
                ...
                # Merge history findings into all findings
        ...
        if history_findings:
            click.echo(f"📜 From git history: {len(history_findings)}")
...
def scan_history(ctx, path: str, branch: Optional[str], all_branches: bool,
    ...
    try:
        ...
        # Generate purge file (BFG-compatible format)
        with open(output, "w") as f:
            f.write("# Secrets to purge - Generated by DeployGuard\n")
            f.write(f"# Generated: {datetime.now().isoformat()}\n")
            f.write("# Format: secret_value==>replacement (or just secret_value for ***REMOVED***)\n\n")
            for secret in secrets:
                if use_env_vars and secret.suggested_env_var:
                    f.write(f"{secret.value}==>${{{secret.suggested_env_var}}}\n")
                else:
            ...
            with open(env_template, "w") as f:
                ...
                for secret in secrets:
                    ...
                    if var_name and var_name not in seen_vars:
                        ...
                        files_str = ", ".join(secret.files[:3])
                        f.write(f"# Type: {secret.secret_type}\n")
                        f.write(f"# Found in: {files_str}\n")
        ...
        # Exit with appropriate code (0 = no critical, 2 = critical secrets found)
        if severity_counts["critical"] > 0:
            click.echo(f"\n⚠️  Found {severity_counts['critical']} CRITICAL secrets. Review and clean before publishing.")
            raise SystemExit(2)
        else:
        ...
        raise  # Let SystemExit pass through without catching
    except click.exceptions.Exit:
        raise  # Let click Exit pass through
    except Exception as e:
        click.echo(f"\n❌ Error: {e}", err=True)
        import traceback
        traceback.print_exc()

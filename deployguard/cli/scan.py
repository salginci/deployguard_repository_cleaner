...
def scan_history(ctx, path: str, branch: Optional[str], all_branches: bool,
    ...
    click.echo(f"\n📜 Scanning git history: {os.path.abspath(path)}")
    click.echo("=" * 60)
    
    if all_branches:
        click.echo("🔀 Scanning all branches...")
    elif branch:
        click.echo(f"🔀 Scanning branch: {branch}")
    ...
    try:
        ...
        # Display findings
        click.echo(f"\n🔍 Found {len(secrets)} unique secrets in history:")
        click.echo("-" * 60)
        ...
        for i, secret in enumerate(secrets[:20], 1):
            ...
            click.echo(f"   📝 Suggested env var: {secret.suggested_env_var}")

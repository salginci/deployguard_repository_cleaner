...
class GitHistoryCleaner:
    ...
    def _suggest_env_var_name(
        ...
        type_prefixes = {
            SecretType.AWS_KEY: "DG_AWS_ACCESS_KEY_ID",
            SecretType.AWS_SECRET: "DG_AWS_SECRET_ACCESS_KEY",
            SecretType.GITHUB_TOKEN: "DG_GITHUB_TOKEN",
            SecretType.API_KEY: "DG_API_KEY",
            SecretType.DATABASE_URL: "DG_DATABASE_URL",
            SecretType.PASSWORD: "DG_DB_PASSWORD",
            SecretType.PRIVATE_KEY: "DG_PRIVATE_KEY",
            SecretType.JWT_TOKEN: "DG_JWT_SECRET",
            SecretType.OAUTH_SECRET: "DG_OAUTH_CLIENT_SECRET",
            SecretType.ENCRYPTION_KEY: "DG_ENCRYPTION_KEY",
        }
        
        # Handle both enum and string types
        if isinstance(secret_type, SecretType):
            base_name = type_prefixes.get(secret_type, "DG_SECRET")
        else:
            # It's a string - try to create a sensible name
            type_str = str(secret_type).upper().replace("-", "_").replace(" ", "_")
            base_name = f"DG_{type_str}"
        
        # Try to extract service name from file path
        file_name = Path(file_path).stem.upper()
        if file_name and file_name not in ["CONFIG", "SETTINGS", "ENV", "SECRETS", "APPSETTINGS"]:
            # Only add file context if base_name is generic
            if base_name == "DG_SECRET":
                return f"DG_{file_name}_SECRET"
